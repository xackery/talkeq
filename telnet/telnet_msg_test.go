...
func TestConvert64Links(t *testing.T) {
	//[\x1200046F000000000000000000000000000000000000000Mask of Tinkering\x12]
	//latest looks like this
	//\x1200F406000000000000000000000000000000000000000000B519D6B0Ring of Prophetic Visions\x12'\n"
	client, err := New(context.Background(), config.Telnet{
	...
	messages := []test{
		...
		{input: "\x1200046F00000000000000000000000000000000000000000000000000Mask of Tinkering\x12 0.8.0 style", output: "http://test.com?itemid=1135 (Mask of Tinkering) 0.8.0 style"},
		{input: "\x1200087E000000000000000000000000000000000000000000B519D6B0Ring of Prophetic Visions\x12\n", output: "http://test.com?itemid=62470 (Ring of Prophetic Visions)"},
	...
	for _, message := range messages {
		result := client.convertLinks(message.input)
		if result != message.output {
			t.Fatalf("convertLinks failed: got %s, wanted %s", result, message.output)
		}
	}
}
