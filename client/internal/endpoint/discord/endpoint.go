...
func (e *Endpoint) onConnect(ctx context.Context) (err error) {
	...
	if e.config == nil || e.config.Discord == nil {
		err = fmt.Errorf("no configuration found")
		return
	}
	if len(e.config.Discord.ServerID) == 0 {
		err = fmt.Errorf("ServerID must be configured")
		return
	}
	if len(e.config.Discord.Token) == 0 {
		err = fmt.Errorf("Token must be configured")
		return
	}
	e.conn, err = discordgo.New("Bot " + e.config.Discord.Token)
	if err != nil {
		err = errors.Wrap(err, "failed to connect")
		return
	}
	e.conn.StateEnabled = true
	e.conn.AddHandler(e.ChannelMessageRead)
	err = e.conn.Open()
	if err != nil {
		err = errors.Wrap(err, "failed to open connection")
		return
	}
